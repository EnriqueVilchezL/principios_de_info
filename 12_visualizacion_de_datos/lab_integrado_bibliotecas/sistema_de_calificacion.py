...
class GrupoCalificacion:
    ...
    def _recalcular_puntos(self):
        ...
        self.pruebas_fallidas = [(n, valor_por_prueba) for n, _ in self.pruebas_fallidas]
        self.pruebas_no_implementadas = [(n, valor_por_prueba) for n, _ in self.pruebas_no_implementadas]
    ...
    def obtener_estadisticas(self) -> dict:
        ...
        total_pruebas = (len(self.pruebas_pasadas) + 
        ...
        return {
            ...
            "porcentaje": (nota_obtenida / self.valor_maximo * 100) if self.valor_maximo > 0 else 0
        }
    ...
    def mostrar_resumen(self, verbose: bool = True):
        ...
        print(f"Pruebas: {stats['pasadas']}/{stats['total_pruebas']} pasadas "
...
class SistemaCalificacion:
    ...
    def mostrar_resumen_por_seccion(self):
        ...
        def mostrar_seccion(nombre: str, grupos: list[GrupoCalificacion]):
            ...
            total_pruebas = sum(g.obtener_estadisticas()['total_pruebas'] for g in grupos)
            pruebas_pasadas = sum(g.obtener_estadisticas()['pasadas'] for g in grupos)
            
            porcentaje = (nota_seccion / valor_seccion * 100) if valor_seccion > 0 else 0
            ...
            for grupo in grupos:
                ...
                print(f"      • {grupo.nombre}: {stats['nota_obtenida']:.2f}% / {stats['valor_maximo']:.2f}%")
        ...
        print("\n" + "─" * 70)
        print(f"🎓 CALIFICACIÓN TOTAL: {nota_total:.2f}% / {valor_total:.2f}% ({porcentaje_global:.1f}%)")
        print("=" * 70)
